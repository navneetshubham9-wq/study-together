...
const subjectAssets = {
    ...
    biology: [ {name: "Human Skeleton", url: "assets/subjects/human_skeleton.pdf"}, {name: "Respiratory System", url: "assets/subjects/respiratory_system.pdf"}, {name: "Human Heart", url: "assets/subjects/human_heart.pdf"}, {name: "Plant Cell", url: "assets/subjects/plant_cell.pdf"}, {name: "Human Brain", url: "assets/subjects/human_brain.pdf"}, {name: "Digestive System", url: "assets/subjects/digestive_system.pdf"}, {name: "DNA Structure", url: "assets/subjects/dna_structure.pdf"}, {name: "Human Eye", url: "assets/subjects/human_eye.pdf"} ],
    chemistry: [ {name: "Periodic Table", url: "assets/subjects/periodic_table.pdf"}, {name: "Distillation Process", url: "assets/subjects/distillation.pdf"}, {name: "Atom Model", url: "assets/subjects/atom_model.pdf"} ],
    physics: [ {name: "Electric Circuit", url: "assets/subjects/electric_circuit.pdf"}, {name: "Magnetic Field", url: "assets/subjects/magnetic_field.pdf"}, {name: "Light Prism", url: "assets/subjects/light_prism.pdf"}, {name: "Pendulum", url: "assets/subjects/pendulum.pdf"} ],
    maths: [ {name: "Graph Paper", url: "assets/subjects/graph_paper.pdf"}, {name: "Protractor", url: "assets/subjects/protractor.pdf"}, {name: "Unit Circle", url: "assets/subjects/unit_circle.pdf"}, {name: "Geometry Shapes", url: "assets/subjects/geometry_shapes.pdf"} ],
    commerce: [ {name: "Supply & Demand", url: "assets/subjects/supply_demand.pdf"}, {name: "Business Cycle", url: "assets/subjects/business_cycle.pdf"} ]
};
...
async function loadAssetToCanvas(url, name) {
    ...
    } catch(e) { showNotification(`Failed to load ${name}. Make sure the file exists!`, "danger"); }
}
...
function loadSubjectAssets(cat) {
    ...
    subjectAssets[cat].forEach(asset => {
        ...
        btn.onclick = () => { loadAssetToCanvas(asset.url, asset.name); }; subjectAssetsList.appendChild(btn);
    });
}
...
function floodFill(startX, startY, fillColorHex, emit=false) {
    if(!ctx) return;
    const hex = fillColorHex.replace('#',''); const fillR = parseInt(hex.substring(0,2), 16); const fillG = parseInt(hex.substring(2,4), 16); const fillB = parseInt(hex.substring(4,6), 16); const fillA = 255;
    ...
    const startPos = (sy * width + sx) * 4; const startR = data[startPos]; const startG = data[startPos + 1]; const startB = data[startPos + 2]; const startA = data[startPos + 3];
    ...
    while (pixelStack.length) {
        const newPos = pixelStack.pop(); const x = newPos[0]; let y = newPos[1]; let pixelPos = (y * width + x) * 4;
        while (y-- >= 0 && matchColor(pixelPos)) { pixelPos -= width * 4; }
        pixelPos += width * 4; ++y; let reachLeft = false; let reachRight = false;
    ...
    ctx.putImageData(imageData, 0, 0); if(emit) socket.emit("wb-fill", {room: currentRoom, x: sx, y: sy, color: fillColorHex});
}
...
function drawFreehand(x0, y0, x1, y1, color, size, toolType, emit = false) {
  ...
  if(toolType === 'eraser') {
      ...
      ctx.fillStyle = color;
      for (let i = 0; i < size * 2; i++) { const offsetX = x1 + (Math.random() * size - size/2); const offsetY = y1 + (Math.random() * size - size/2); ctx.fillRect(offsetX, offsetY, 2, 2); }
  } else {
...
function drawShapeObj(x0, y0, x1, y1, type, color, size, emit = false) {
  ...
  if(type === 'line') { ctx.moveTo(x0, y0); ctx.lineTo(x1, y1); }
  else if(type === 'arrow') { ctx.moveTo(x0, y0); ctx.lineTo(x1, y1); let angle = Math.atan2(y1-y0, x1-x0); ctx.lineTo(x1 - size*3 * Math.cos(angle - Math.PI/6), y1 - size*3 * Math.sin(angle - Math.PI/6)); ctx.moveTo(x1, y1); ctx.lineTo(x1 - size*3 * Math.cos(angle + Math.PI/6), y1 - size*3 * Math.sin(angle + Math.PI/6)); }
      ...
      for(let i=0; i<=sides; i++) {
          ...
          if(type === 'star' && i<sides) { let ix = x0 + (r/2.5) * Math.cos((i+0.5) * step - Math.PI/2); let iy = y0 + (r/2.5) * Math.sin((i+0.5) * step - Math.PI/2); if(i===0) ctx.moveTo(cx, cy); else ctx.lineTo(cx, cy); ctx.lineTo(ix, iy); } else { if(i===0) ctx.moveTo(cx, cy); else ctx.lineTo(cx, cy); }
      }
      ctx.closePath();
  }
  else if(type === 'cube') {
      let d = Math.min(Math.abs(w), Math.abs(h)) * 0.4; ctx.rect(x0, y0+d, w-d, h-d); ctx.moveTo(x0+d, y0); ctx.lineTo(x1, y0); ctx.lineTo(x1, y1-d); ctx.lineTo(x0+d, y1-d); ctx.closePath(); ctx.moveTo(x0, y0+d); ctx.lineTo(x0+d, y0); ctx.moveTo(x1-d, y0+d); ctx.lineTo(x1, y0); ctx.moveTo(x1-d, y1); ctx.lineTo(x1, y1-d); ctx.moveTo(x0, y1); ctx.lineTo(x0+d, y1-d);
  }
  else if(type === 'cylinder') { let rX = Math.abs(w/2), rY = Math.abs(h * 0.15); ctx.ellipse(x0 + w/2, y0 + rY, rX, rY, 0, 0, 2 * Math.PI); ctx.moveTo(x0, y1 - rY); ctx.ellipse(x0 + w/2, y1 - rY, rX, rY, 0, 0, Math.PI); ctx.moveTo(x0, y0 + rY); ctx.lineTo(x0, y1 - rY); ctx.moveTo(x1, y0 + rY); ctx.lineTo(x1, y1 - rY); }
  else if(type === 'cone') { let rX = Math.abs(w/2), rY = Math.abs(h * 0.15); ctx.ellipse(x0 + w/2, y1 - rY, rX, rY, 0, 0, 2 * Math.PI); ctx.moveTo(x0 + w/2, y0); ctx.lineTo(x0, y1 - rY); ctx.moveTo(x0 + w/2, y0); ctx.lineTo(x1, y1 - rY); }
  else if(type === 'sphere') { let r = Math.sqrt(w*w + h*h); ctx.arc(x0, y0, r, 0, 2*Math.PI); ctx.moveTo(x0-r, y0); ctx.ellipse(x0, y0, r, r*0.3, 0, 0, 2*Math.PI); }
