...
const roomHostUid = new Map(); // NAYA: Host ka UID track karne ke liye
...
const roomOfficeState = new Map(); // NAYA: Office State
...
io.on("connection", socket => {
  ...
  socket.on("join-room", info => {
    ...
    if (!roomHosts.has(room) || roomHosts.get(room) === null) {
      ...
      roomHostUid.set(room, uid); // Store Host UID
